...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  ## Synced folders
  config.vm.synced_folder "./vagrant", "/vagrant", type: SYNC
  config.vm.synced_folder "./drupal", "/drupal", type: SYNC
  ...
  config.vm.provision "shell", inline: "cp /vagrant/xdebug/xdebug.ini /etc/php5/apache2/conf.d/xdebug.ini"
  config.vm.provision "shell", inline: "apache2ctl restart"
  config.vm.provision "shell", inline: "cp -R /var/www/drupal/* /drupal/"
